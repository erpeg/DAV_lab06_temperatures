...
def main():
    ...
    cities_to_legend =[]
    cities_to_legend += [list(city.keys()) for city in grouped_data.values()]
    cities_to_legend = [item for sublist in cities_to_legend for item in sublist]
    cities_to_legend.sort()
    colors = cm.gist_rainbow(np.linspace(0, 1, len(cities_to_legend)))   # create array of colors
    colors = np.delete(colors, -1, axis=1)  # remove alpha column
    colors[:, 0:3] *= 0.8    # darken colors

    # country_id:color
    dict_of_colors = {}
    for city, color in zip(cities_to_legend, colors):
        dict_of_colors[city] = color
    ...
    for country, city_data in grouped_data.items():
        for city, temps_per_year in city_data.items():
            list_for_legend.append(city)
            # dict_for_legend[trans_dict[country]] = dict_of_colors[country]  # translate country_ids to full countries in new dict
            l, = axs[placing_plots[country][0], placing_plots[country][1]].plot(list(temps_per_year.keys()), list(temps_per_year.values()), color=dict_of_colors[city], alpha=0.8, linewidth=2, label=city)
    ...
    list_for_legend, axes = zip(*sorted(zip(list_for_legend, axes), key=lambda k: k[0]))
    ...
    if args.show_save == '0':
        ...
        plt.savefig('plots/task5d.png', bbox_inches='tight')
